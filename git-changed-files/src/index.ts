...
integration.addBuildEventHandler("onSuccess", ({utils: {status, git}}) => {
  ...
  const markdownList = createMarkdown(git.modifiedFiles, process.env.DEPLOY_PRIME_URL);
 ...
 * @returns string[]
 */
export function createMarkdown(modifiedFiles: readonly string[], netlifyURL: string) : string[] {
